...
def user_monitor(username):
    try:
        ...
        usname = username
        sql = "select e.menu_cn_name as loginNum from user_department a  LEFT JOIN user_dep_rela b on a.dep_id = b.dep_id left join user_role_rela c on b.user_id = c.user_id left join menu_privilege d on c.role_id = d.role_id left join sys_menu e on d.menu_id = e.menu_id where  e.super_menu_id not in(1,2,16,52,45,154) and e.menu_state = 1 and a.dep_name = '{0}'".format(username)
